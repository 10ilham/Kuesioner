...
class Kinerja extends My_Controller
{
    ...
    function detail_kelola()
    {
        ...
        $ta = $data['ta'];
        $check = $this->kinerja_model->getPeriodeByST('periode', ['semester' => $sem, 'tahun_akademik' => $ta])->row_array();
        $data['responden'] = $this->db->query("SELECT COUNT(distinct username) as total, semester, tahun_akademik FROM respon, periode where respon.id_periode=periode.id_periode and respon.id_periode='" . $check['id_periode'] . "' AND typeaspek = 'kelola'")->row_array();
